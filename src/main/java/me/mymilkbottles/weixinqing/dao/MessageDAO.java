...
public interface MessageDAO {
    ...
    @Select({"select " + ALL_FIELD + " from " + TABLE_NAME + " where id = #{id} and is_delete != 1"})
    public Message getMessageById(@Param("id") int id);

    @Select({"select count(id) from " + TABLE_NAME + " where advicer = #{id} and is_delete != 1 and is_read = 0"})
    public int getUserUnreadMessageCount(int id);

    @Select({"select count(id) from " + TABLE_NAME + " where advicer = #{id} and is_delete != 1"})
    public int getUserMessageCount(int id);

    @Select({"select count(id) from " + TABLE_NAME + " where advicer = #{id} and is_delete != 1 and is_read = 0 and msg_type = #{type}"})
    public int getUserUnreadTypeMessageCount(@Param("id") int id, @Param("type") int type);

    @Select({"select count(id) from " + TABLE_NAME + " where advicer = #{id} and is_delete != 1 and msg_type = #{type}"})
    public int getUserTypeMessageCount(@Param("id") int id, @Param("type") int type);

    @Select({"select " + ALL_FIELD + " from " + TABLE_NAME + " where advicer = #{id} and is_delete != 1 and is_read = 0 order by stime desc limit s, e"})
    public List<Message> getUserUnreadMessage(@Param("id") int id, @Param("s") int start, @Param("e") int end);

    @Select({"select " + ALL_FIELD + " from " + TABLE_NAME + " where advicer = #{id} and is_delete != 1 order by stime desc limit s, e"})
    ...
    @Select({"select " + ALL_FIELD + " from " + TABLE_NAME + " where advicer = #{id} and is_delete != 1 and msg_type = #{type} order by stime desc limit s, e"})
